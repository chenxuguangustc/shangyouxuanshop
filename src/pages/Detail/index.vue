      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(attr, index) in spuSaleAttrList" :key="attr.id">
                ...
                <dd
                  ...
                  :key="value.id"
                  @click="clickSaleAttr(value, attr.spuSaleAttrValueList, index)"
                >
                  {{ value.saleAttrValueName }}
                  <span
                    v-show="value.isChecked === '1'"
                    class="cancel"
                    title="点击取消选择"
                    :style="cancelBg"
                    >&nbsp;</span
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- input事件是输入的时候同步触发，blur是失去焦点的时候触发（不管内容有没有改变），
                 change是失去焦点的时候触发，内容必须触发 -->
...
export default {
  ...
  methods: {
    // 排他思想，将所有的选项都传进去，然后将点击的那个变成不一样的
    ...
    async addToCart() {
      ...
      // 发请求给后台数据库的购物车添加数据，dispatch分发触发，其实就是在调用响应的actions函数
      // this.$store.dispatch('addCart', {skuId, skuNum})就是函数调用表达式，结果就是actions当中异步函数调用
      // 的返回值，异步函数的返回值一定是Promise，既然是Promise，那就可以加await
      try {
        // 只要在try里面就一定是成功的，
        // 路由传参一般传的都是简单的数据，对象一般都是用JSON转换一下
        // 如果两个路由之间传递的是复杂数据，建议使用存储手段

        // 以后碰到传递的是简单的数据，使用路由传单，复杂的数据采用存储手段
        // localstorage和sessionStorage，他们都有四个方法，setItem, getItem, removeItem, clear
        // 区别：localstorage是永久存储，sessionStorage浏览器关闭就没了

        // 将商品信息放到sessionStorage中，这样不同组件都能访问到，但是不能存储对象，所以转换成字符串
        sessionStorage.setItem('SKUINFO_KEY', JSON.stringify(this.skuInfo))

        const result = await this.$store.dispatch('addCart', {skuId, skuNum})
        alert('添加成功')

        // 成功后进行跳转，只能用编程式导航，query参数以问号为分隔符，Params需要用一下/
        this.$router.push('/AddCartSuccess?skuNum='+this.skuNum)
      } catch (error) {
        // 失败的原因和vuex里面return的失败的原因是一样的
        alert(error.message)
      }
      // 根据添加的结果来决定要不要跳转到购物车页面
    }
  },
